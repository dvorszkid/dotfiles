...
local clientKeys = awful.util.table.join(
	awful.key({ modkey }, "Escape", function(c)
		c:kill()
	end, { description = "kill", group = "client modifiers" }),
	awful.key({ modkey, "Control" }, "f", function(c)
		c.fullscreen = not c.fullscreen
	end, { description = "toggle fullscreen", group = "client modifiers" }),
	awful.key(
		{ modkey },
		"space",
		awful.client.floating.toggle,
		{ description = "toggle floating", group = "client modifiers" }
	),
	awful.key({ modkey }, "o", function(c)
		c:move_to_screen()
	end, { description = "move to next screen", group = "client modifiers" }),
	awful.key({ modkey }, "t", function(c)
		c.ontop = not c.ontop
	end, { description = "toggle ontop", group = "client modifiers" }),
	awful.key(
		{ modkey, "Control" },
		"t",
		awful.titlebar.toggle,
		{ description = "toggle titlebar", group = "client modifiers" }
	),
	awful.key({ modkey }, "s", function(c)
		c.sticky = not c.sticky
	end, { description = "toggle sticky", group = "client modifiers" }),
	awful.key({ modkey }, "n", function(c)
		c.minimized = true
	end, { description = "minimize", group = "client modifiers" }),
	awful.key({ modkey }, "m", function(c)
		c.maximized = not c.maximized
	end, { description = "toggle maximixed", group = "client modifiers" }),
	awful.key({ modkey, "Control" }, "m", function(c)
		c:swap(awful.client.getmaster())
	end, { description = "set as master", group = "client modifiers" })
)
...
local globalKeys = awful.util.table.join(
	...
	awful.key(
		{ modkey, "Control" },
		"n",
		apps.func.notifications_toggle,
		{ description = "toggle notifications", group = "awesome" }
	),

	-- Prompts
	awful.key({ modkey }, "r", function()
		awful.spawn(apps.cmd.runcmd)
	end, { description = "run command", group = "prompts" }),
	awful.key({ modkey, altkey, "Control" }, "r", function()
		awful.prompt.run(
			{ prompt = "Run Lua code: " },
	...
	end, { description = "focus previous", group = "screen handling" }),
	awful.key({ modkey, altkey }, "]", function()
		awful.screen.focus_relative(1)
	end, { description = "focus next", group = "screen handling" }),

	-- Tag browsing
	awful.key({ modkey }, "`", function()
		mouse.screen.selected_tag.selected = false
	end, { description = "select none", group = "tag handling" }),
	...
	end, { description = "move left", group = "tag handling" }),
	awful.key({ modkey, "Shift" }, "]", function()
		lain.util.move_tag(1)
	end, { description = "move right", group = "tag handling" }),

	awful.key({ modkey, "Shift" }, "l", function()
		move_tag_to_screen("right")
	end, { description = "move to screen right", group = "tag handling" }),
	awful.key({ modkey, "Shift" }, "h", function()
		move_tag_to_screen("left")
	end, { description = "move to screen left", group = "tag handling" }),
	awful.key({ modkey, "Shift" }, "j", function()
		move_tag_to_screen("down")
	end, { description = "move to screen down", group = "tag handling" }),
	awful.key({ modkey, "Shift" }, "k", function()
		move_tag_to_screen("up")
	end, { description = "move to screen up", group = "tag handling" }),

	awful.key({ modkey, "Shift" }, "n", function()
		lain.util.add_tag()
	end, { description = "new", group = "tag handling" }),
	awful.key({ modkey, "Shift" }, "r", function()
		lain.util.rename_tag()
	end, { description = "rename", group = "tag handling" }),
	awful.key({ modkey, "Shift" }, "d", function()
		lain.util.delete_tag()
	end, { description = "delete", group = "tag handling" }),
	...
	awful.key({ modkey }, "Tab", function()
		awful.spawn(apps.cmd.windowswitch)
	end, { description = "select window", group = "client handling" }),
	...
	awful.key(
		...
		{ description = "jump to urgent", group = "client handling" }
	),
	awful.key({ modkey, "Shift" }, "k", function()
		awful.client.swap.byidx(-1)
	end, { description = "move to previous", group = "client handling" }),
	awful.key({ modkey, "Shift" }, "j", function()
		awful.client.swap.byidx(1)
	end, { description = "move to next", group = "client handling" }),
	...
	end, { description = "select next", group = "layout handling" }),
	awful.key({ modkey, "Shift", "Control" }, "space", function()
		awful.layout.inc(awful.layout.layouts, -1)
	end, { description = "select previous", group = "layout handling" }),

	-- User programs
	awful.key({ modkey }, "Return", function()
		awful.spawn(apps.term.tmux)
	end, { description = "terminal with tmux", group = "programs" }),
	awful.key({ modkey, "Shift" }, "Return", function()
		awful.spawn(apps.cmd.terminal)
	end, { description = "terminal without tmux", group = "programs" }),
	awful.key({}, "XF86Calculator", function()
		awful.spawn(
			apps.cmd.calculator,
			{ tag = awful.screen.focused().selected_tag, intrusive = true, floating = true }
		)
	end, { description = "calculator", group = "programs" }),
	awful.key({ "Control" }, "Escape", function()
		awful.spawn(apps.term.procmon)
	end, { description = "process monitor", group = "programs" }),
	awful.key({}, "Print", function()
		awful.spawn(apps.cmd.screenshot)
	...
	awful.key(
		...
		apps.func.volume_increase,
		{ description = "increase volume", group = "volume" }
	),
	awful.key(
		{},
		"XF86AudioLowerVolume",
		apps.func.volume_decrease,
	...
	awful.key({ "Control" }, "XF86AudioRaiseVolume", function()
		awful.spawn(apps.cmd.amplifier_volume_up)
	end, { description = "increase amplifier volume", group = "volume" }),
	awful.key({ "Control" }, "XF86AudioLowerVolume", function()
		awful.spawn(apps.cmd.amplifier_volume_down)
	end, { description = "decrease amplifier volume", group = "volume" }),
	awful.key({ "Control" }, "XF86AudioMute", function()
		awful.spawn(apps.cmd.amplifier_toggle)
	end, { description = "toggle amplifier on / off", group = "volume" }),

	-- Spotify control
	awful.key({}, "XF86AudioPlay", function()
		awful.spawn.with_shell(apps.cmd.music_play)
	end, { description = "play / pause", group = "spotify" }),
	awful.key({}, "XF86AudioStop", function()
		awful.spawn.with_shell(apps.cmd.music_stop)
	end, { description = "stop", group = "spotify" }),
	awful.key({}, "XF86AudioPrev", function()
		awful.spawn.with_shell(apps.cmd.music_prev)
	end, { description = "previous", group = "spotify" }),
	awful.key({}, "XF86AudioNext", function()
		awful.spawn.with_shell(apps.cmd.music_next)
...
local setupUi = function(ui)
	...
	ui.calendar_widget:connect_signal("button::press", function(_, _, _, button)
		if button == 1 then
			awful.spawn(apps.cmd.calendar)
		end
		if button == 3 then
			ui.calendar_popup.toggle()
		end
		if button == 4 then
			ui.calendar_popup.next()
		end
		if button == 5 then
			ui.calendar_popup.previous()
		end
	end)
end
